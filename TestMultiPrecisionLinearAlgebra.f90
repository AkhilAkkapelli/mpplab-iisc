PROGRAM test_multi_precision_linear_algebra
  ...
  ! print*, range
  ! DO i = 1, 10
  !   print('(A,I0,5I20)'), "mpf", i, create_random_mpf(range, seed)
  ! END DO
...
CONTAINS
  ...
  SUBROUTINE test_new_mpf_vector_from_mpfs()
    ...
    seed = 1234567
    range_mpf = new_mpf_from_mpi_exp(new_mpi_from_coeffs((/ MULTI_PRECISION_BASE-1, MULTI_PRECISION_BASE-1, 0_8, 0_8 /)), 0_1)
    ALLOCATE(mpf_elements(4))

    mpf_elements(1) = create_random_mpf(range_mpf, seed)
    mpf_elements(2) = create_random_mpf(new_mpf_from_integer(MULTI_PRECISION_BASE), seed)
    mpf_elements(3) = create_random_mpf(new_mpf_from_integer(-MULTI_PRECISION_BASE), seed)
    mpf_elements(4) = create_random_mpf(-range_mpf, seed)

    vec = new_mpf_vector_from_mpfs(mpf_elements)

    DO i = 1, SIZE(mpf_elements)
      print('(A,I0,5I20)'), "mpf", i, mpf_elements(i)
    END DO

    print*, "exp", vec%exponent
    CALL assert(vec%n == 4, "Vector Construction (Complex): Size")

    print*, "mpf", mpf_elements(1)
    print*, "vec", vec%mantissas(1)

    CALL assert(mpf_value_equal(new_mpf_from_mpi_exp(vec%mantissas(1), vec%exponent), mpf_elements(1)), &
                "Vector Construction (Complex): Value 1 preserved")

    print*, "mpf", mpf_elements(2)
    print*, "vec", vec%mantissas(2)
    CALL assert(mpf_value_equal(new_mpf_from_mpi_exp(vec%mantissas(2), vec%exponent), mpf_elements(2)), &
                "Vector Construction (Complex): Value 2 preserved")

    print*, "mpf", mpf_elements(3)
    print*, "vec", vec%mantissas(3)
    CALL assert(mpf_value_equal(new_mpf_from_mpi_exp(vec%mantissas(3), vec%exponent), mpf_elements(3)), &
                "Vector Construction (Complex): Value 3 preserved")

    print*, "mpf", mpf_elements(4)
    print*, "vec", vec%mantissas(4)
    CALL assert(mpf_value_equal(new_mpf_from_mpi_exp(vec%mantissas(4), vec%exponent), mpf_elements(4)), &
