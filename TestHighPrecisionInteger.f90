...
SUBROUTINE test_normalization()
  ...
  integer(8) :: input_coeffs(COEFFS_LIMIT)
  ...
  do k= 1, ntests
    ! read line: [Ip coeffs] [sign] [output coeffs]
    read(unit_num, *)input_coeffs, exp_sign, expec_coeffs
    mpi_val%coeffs= input_coeffs
  ...
  if (npass == ntests) then
    print '(A,I0,A,I0,A)', "[PASS] Normalization: Randomly Generated cases: ", npass, "/", ntests, " cases passed" 
  else
    print '(A,I0,A,I0,A)', "[FAIL] Normalization: Randomly Generated cases: ", npass, "/", ntests, " cases passed" 
  end if
